...
public:
        ...
        , _rampInteractive(NULL)
    {
        ...
        assert((!_srcClip && getContext() == eContextGenerator) || (_srcClip && (!_srcClip->isConnected() || _srcClip->getPixelComponents() == ePixelComponentRGB || _srcClip->getPixelComponents() == ePixelComponentRGBA || _srcClip->getPixelComponents() == ePixelComponentAlpha)));
...
#ifdef SENOISE_VORONOI
        assert(_noiseType && _noiseSize && _noiseZ && _noiseZSlope && _voronoiType && _jitter && _fbmScale && _octaves && _lacunarity && _gain);
#else
        assert(_noiseType && _noiseColored && _noiseSize && _noiseZ && _noiseZSlope && _octaves && _lacunarity && _gain);
#endif
                               ...
                               const RenderArguments& args)
{
    ...
    auto_ptr<const Image> src((_srcClip && _srcClip->isConnected()) ? _srcClip->fetchImage(time) : 0);
    if (src.get()) {
...
SeNoisePlugin::resetCenter(double time)
{
    ...
    if ((rod.x1 <= kOfxFlagInfiniteMin) || (kOfxFlagInfiniteMax <= rod.x2) || (rod.y1 <= kOfxFlagInfiniteMin) || (kOfxFlagInfiniteMax <= rod.y2)) {
        return;
    }
    ...
    Matrix3x3 Rinv = (ofxsMatRotation(-rot) * ofxsMatSkewXY(skewX, skewY, skewOrder) * ofxsMatScale(scale.x, scale.y));
    OfxPointD newCenter;
